...
declare class HubSpotClient {
  ...
  contacts: IContactsApi; // IContactsApi;
...
interface IContactsApi {
  createOrUpdateContact: (contact: any) => Promise<any>;
...
interface IFilesApi {
  getFilesInFolder: (
    folderId: number,
    opts?: any
  ) => Promise<{ limit: number; objects: any[] }>;
  getFolders: (
...
interface ILayoutsApi {
  getLayouts: (
    opts?: ILayoutsApi.IGetLayoutOpts
  ) => Promise<{ limit: number; objects: ILayoutsApi.ILayoutEntity[] }>;
